...
const fetchUsers = async(): Promise<UsersType> => {
  try {
    ...
    // : async 처리된 함수는 내부에서 어떤 값을 반환(return)하든
...
const showModal = (user: User) => {
  ...
  modal.style.display = 'block'; // display:none을 보여지게 표시 - 모달 창 표시
  }
}
...
const addEventListeners = (users: UsersType) => {
  ...
  if (modal && closeModal) {
    ...
    // cf) document: window에 로드되는 HTML 문서 그자체
