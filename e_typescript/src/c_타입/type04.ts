//! === Union 타입 ===
// : 여러 타입 '중' 하나가 될 수 있는 값을 나타내는 방법 (확장)
...
let kja:AdminUser = {
  ...
  name: '권지애',
  address: '부산시 기장군'
} // User거나 Admin거나 둘 중 하나는 적혀있어야 함

//? 타입 별칭에서 union 타입 사용 시
// : 정확한 타입 지정을 위해 '타입 가드'를 사용 (제한)
...
function getAge(age: Union) {
  // 나이가 전달될 경우
  // - 숫자: 소수점 자리가 없도록 반올림 + 문자열로 반환 (.toFixed() - 숫자 only)
  // - 문자: 대문자로 변환하여 반환 (.toUpperCase() - 문자 only)

  // + 유니언 타입의 변수는 포함된 타입에서 모두 포함되는 속성과 메서드만 사용 가능!
