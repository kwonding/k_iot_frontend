...
select(); // 개구리 is guest
select('권지애'); // 개구리 is 권지애 (name으로 감지됨)
select('강사'); // 개구리 is 강사 (name에 할당)

// cf) 선택적 매개변수와 기본 매개변수 혼합 사용 시
// 1. 선택적 매개변수는 반드시!! 필수 매개변수(인자로 전달하는 값) 뒤에 작성 !!!
// 2. 기본 매개변수는 필수, 선택적 매개변수의 양쪽 어디든 작성 가능
// 3. 선택적 매개변수가 앞서는 경우 생략하려면 반드시 undefined값 전달해야함
select(undefined, '권지애'); // 권지애 is guest
