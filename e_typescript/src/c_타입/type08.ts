...
// - 컴파일러에게 '이 데이터의 타입을 내가(개발자가)! 지정한 타입으로 간주해라!'라는 지시
...
// console.log(length); // undefined - boolean값 length(X)

let length: number = (someValue as string).length;
console.log(length); // 16

//? 타입 단언 사용 예시
// : 웹 개발에서 DOM 요소를 조작할 때(querySelector), 특정 DOM 요소를 구체적인 HTMLElement 타입으로 단언!
// - TS가 자동으로 해당 DOM 요소의 정확한 타입을 추론할 수 X 따라서 개발자가 단언해줘야 함
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (button) {
    ...
    // button.disabled = false; // 아직 타입이 'HTMLElement' 라서 .disabled 사용불가
    (button as HTMLButtonElement).disabled = true;
  }
  // TS 컴파일러는 실시간 코드 변환을 하지 X
  // : 코드 수정 후 다시 컴파일 실행 필요함
});

//& 2) JSON 문자열
// - 객체(parse) <> JSON(stringify)
// : JSON 문자열을 분석(parse)하여 반환되는 데이터는 any 타입으로 간주

const jsonString = '{"name": "KJA", "age": 30}';
...
console.log(typeof userData2); // object 객체 타입