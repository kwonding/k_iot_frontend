...
// : interface 키워드 사용
// - 인터페이스명은 대문자로 시작
// - 객체의 문법 체계 모두 사용 가능 (옵셔널(선택적프로퍼티옵션) ?, 읽기전용 readonly)
interface IUser {
  // 인터페이스명: I+타입명
  // 타입별칭: 타입명Type
  name?: string;
...
class Student implements IUser {
  ...
  //    > "컴파일러! 내가 책임질게! 나중에 반드시 초기화 될거야" (! 사용)
...
// Hello2, my name is 이상은

const student2 = new Student('박진영', 30);
student2.greet('정지훈');
// Hello1, my name is 정지훈
// Hello2, my name is 박진영

//? 인터페이스 확장
// A extends B 키워드
// : A와 B가 같은 형식 (클래스면 클래스 / 인터페이스면 인터페이스)
...
interface Square extends Shape {
  sideLength: number;
  // color: string; - 해당 속성이 생략되어 보이지만 존재함
}
...
//& 1) interface
// - extends 키워드로 확장 가능 (A(interface) extends B(interface))
// - 같은 이름으로 여러 번 선언 가능 (자동 병합됨)
