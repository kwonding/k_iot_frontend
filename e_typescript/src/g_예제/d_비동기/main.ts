...
3. 실제 데이터 요청 실행 후 완료 시 사용자 데이터가 화면에 표시
...
fetchButton?.addEventListener('click', async () => {
  // 비동기 async 함수
  // : 원래 모습 - async function() {}
  // : 화살표 함수 - async () => {}

  //! DOM 요소 가져오기
  const userDataDiv = document.getElementById('userData'); // 메시지 창
  const userIdElement = document.getElementById('userId') as HTMLInputElement; // 인풋창
  ...
  if (userDataDiv) {
    ...
    try {
      ...
      const user = await fetchResponse.json(); // JS객체로 가져오기(변환)
      ...
      `;
      // address는 객체타입 - address안의 street만 가져옴
    } catch(e) {
      //? 데이터 처리 실패 시 메시지 출력
