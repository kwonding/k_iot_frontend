...
& 요구사항 정리
  ...
  3. 할 일 리스트를 관리하는 함수 구현
    - addTodo (추가)
    - completedTodo (수정) - 요소 개수 변화없음: map
    - deleteTodo (삭제) - 요소 개수 변화있음: filter
*/
...
function addTodo(todos: TodoItem[], task: string): TodoItem[] {
  ...
  // cf) todos.push(newTodo); - 가능 / 주소값 변경 X
  // >> 배열의 불변성 (배열은 첫 번째 요소의 주소값이 저장됨)
...
function deleteTodo(todos: TodoItem[], id: number) {
  const changeTodos = todos.filter(todo => todo.id !== id); // 삭제하려는 것만 제외하고 배열에 남기기
...
todos = addTodo(todos, "정처기 공부");
todos = addTodo(todos, "정처산기 공부");
