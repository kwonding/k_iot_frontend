...
// > 데이터 타입을 미리 지정하지 않고, 사용 시점에 데이터 타입을 지정함

//? 제네릭 필요성
// : 코드 중복 최소화, 타입 안정성('컴파일' 시점에 타입 체크)
...
let pair2 = pair<number, string>(345, '안녕하세요');
...
// : myExample은 number타입을 사용하는 IGeneric임
...
class GenericClass<T> {
  ...
  constructor(value: T, addFunc: (x: T, y: T) => T) { // 콜백함수
    this.value = value;
    this.add = addFunc;
  }
}

// let a = (x: number, y: number) => x + y; - 권장하지 않음
