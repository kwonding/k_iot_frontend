...
// > HTML5 웹 스토리지 사양의 일부 도입으로 '쿠키'의 공간 제한을 보완함
...
console.log(userInfoData); // {"name":"권지애","age":30} - JSON임
console.log(JSON.parse(userInfoData)); // {name: '권지애', age: 30}

//? cf) 로컬 스토리지 데이터 반환 시 데이터의 null 값 처리 코드 작성 필수
let noValue = localStorage.getItem('no');
console.log(noValue); // null

// noValue.toLowerCase(); // TypeError: Cannot read properties of null
if (noValue) {
  noValue.toUpperCase();
}
...
console.log(localStorage.getItem('username')); // 삭제했으니 null 뜸
