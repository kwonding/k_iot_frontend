...
//! == 프로젝트 구현 == //
let todos = []; // 배열 리터럴 방식 선언 (빈 배열) - 할 일 목록을 배열 초기화
let wastebasket = []; // 휴지통
let nextId = 1; // 고유 ID값을 위한 전역 변수 - 초기값 1 넣어둔 상태

// 1) 할 일 추가 함수
function addTodo(content) { // 함수 호출 시 인자로 할 일의 내용을 전달받음
  // 새로운 할 일 생성
  const newTodo = {
    id: nextId,       // 전역 변수는 파일 어디에서든 접근 가능 / 지역 변수를 전역 범위에서 접근할 수 없음
    content: content, // 키: 값(매개변수의 데이터 삽입 값)
    completed: false
  };
...
function toggleTodo(id) {
  ...
  //? map: 배열을 순회하여 동일한 기능 적용 후 새로운 배열(let todos, 재할당)에 담아 반환
  //      >> 요소의 개수 변화 없음!
  ...
  todos = todos.map(todo => {
    ...
    if (todo.id === id) {
      ...
      return {...todo, completed: !todo.completed }; // ...todo (id, content 그대로 반환), 수정된 completed만 변경
    }
...
function deleteTodo(id) {
  ...
  // 1, 2, 3, 4, 5 중에서 4를 제거
  // === 4와 일치하지 않는 1, 2, 3, 5만을 새로운 배열로 저장한다
  // >> 4가 삭제된 효과
