...
- id          : 학생 고유 번호
- name        : 학생 이름
- scores      : 각 과목별 성적을 저장하는 객체(배열) { Math: 85, English: 90, Science: 78 }
...
class Student {
  constructor(id, name, scores = {}) {
    ...
    //    , 기본 매개변수 값이 할당됨 (선택적이다)
    // > 필수 전달 데이터보다 뒤에 작성되어야 함!
    this.id = id;
    this.name = name;
    this.scores = scores;
  }
  ...
  getAverageScore() {
    ...
    const sum = values.reduce((acc, cur) => acc + cur, 0); // reduce: 배열을 하나의 값으로
    const avg = sum / values.length;

    //? 숫자.toFixed(소수점자리수)
    // : 해당 소수점 자리수 이하의 자리수를 갖는 "문자열"로 반환
    // > Number()로 형 변환하여 반환!
    return Number(avg.toFixed(2)); // 소수점 2번째자리까지 숫자를 문자열로 바꿔서 반환
  }
}
...
class GradeManagement {
  ...
  getAverageScore() {
    ...
    // return 안에 return이 있으면 앞의 return으로 바로 반환하고 끝나서 안의 return 사용 불가
...
// 학생 추가 예제
gradeSystem.addStudent("권지애", { Math: 90, English: 80, Science: 100});
gradeSystem.addStudent("가가가", { Math: 55, English: 22, Science: 56});
gradeSystem.addStudent("나나나", { Math: 49, English: 46, Science: 88});
gradeSystem.addStudent("다다다", { Math: 66, English: 78, Science: 90});
...
const top = gradeSystem.getTopStudents(50);
console.log('===평균 50점 이상 상위 학생 (내림차순)===');
console.log(top);