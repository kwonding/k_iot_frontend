/*
  ...
  1) 사용자로부터 내용값을 입력받아
  2) list item들을 순회하여 해당 입력값이 포함된 아이템만 
      , display 속성을 '' 빈 문자열 할당 (보여짐)
*/
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  input.addEventListener('input', () => {
    // input 요소의 입력값 가져오기
    let value = input.value.toLowerCase(); // 영어대비
    ...
    items.forEach(item => {
      // HTML요소.textContent: HTML 요소 객체의 내용값임

      // 데이터.includes(값)
      // : 데이터 안에 값이 포함 여부를 boolean 반환
      // >> 데이터: 배열, 문자열 사용 가능
