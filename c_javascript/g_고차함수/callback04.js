...
// - 콜백함수: 지정된 시간 이후에 '단 한 번만' 실행될 함수
// - 시간: 밀리초 단위 (1s === 1000ms)

function runlater(name = '권지애') {
  console.log(`Hello, ${name}^^`);
}

runlater(); // Hello, 권지애^^ - 동기 프로그래밍

setTimeout(runlater, 2000); // Hello, 권지애^^ - 비동기

// setTimeout(runlater('권재헌'), 2000);
// >> The "callback" argument must be of type function. Received undefined
// >> 콜백 함수의 인자는 undefined(비워져있음)여야만 한다!

// cf) 콜백 함수의 인자 전달 방법
// : 세 번째 인자부터 나열되는 값이 함수의 매개변수로 전달
setTimeout(runlater, 3000, '권재헌'); // Hello, 권재헌^^

// cf) 함수 호출: 함수명(전달할 인자 나열);
// >> 호출부 () 작성 시 코드를 읽는 즉시 함수 실행!
// setTimeout(runlater(), 2000, '나나나'); (undefined)

function manyParam(name, age, job) {
  console.log(`${name} is ${age} years old and the job is ${job}`);
}

setTimeout(manyParam, 100, '권지애', 30, 'developer'); // 권지애 is 30 years old and the job is developer
