...
const [passingCountValue, passingAvgValue] = result;

console.log(passingCountValue); // 5
console.log(passingAvgValue); // 89
...
function newArray (numbers) {
  ...
  const add = numbers.map(value => value + 10);

  const odd = add.filter(value => value % 2 !== 0)

  console.log(odd);
}

newArray(numbers); // [ 11, 13, 15 ]
