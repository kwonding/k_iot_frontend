//# === 비동기 프로그래밍 === //
// : asynchronous(비동기적인)
// : 아닐 비, 같을 동, 시기 기 >> 시간이 같이 않음
// - 특정 코드의 실행 완료를 기다리지 않고, 다음 코드를 실행하는 프로그래밍 방식

//? cf) 자바스크립트는 "왜?" 비동기를 쓸까?
// : JS는 단일 스레드를 사용하여 한 번에 한 줄만 실행!
// >> 하지만 자바스크립트 내에서 네트워크 요청(HTTP), 파일 읽기, 타이머, 대용량 연산은 시간 소요가 많음
// >> 해당 동작에서 '동기 프로그래밍' 사용 시 해당 화면이 멈추고(프리징) 클릭도 못 받는 "끊김" 현상이 발생
// >> 그래서 JS는 느린 작업은 밖에 맡겨두고(비동기), 나머지 코드를 계속 진행함
// >> 느린 작업이 끝나면 "끝났어!(성공 or 실패)"라고 응답을 주고 그 결과를 이어서 처리하는 구조

// "부드러운 UI, 빠른 반응성, 높은 처리량"이 핵심 이점!

//? 비동기 프로그래밍 활용 기능
// - 네트워크 요청(서버와의 통신, 컨트롤러 - 서비스 - 리포지토리)
// - 파일 I/O(입출력)
// >> 시간이 오래 소요되는 작업
...
function work() {
  ...
  //     today.now();
  const start = Date.now(); // 현재 날짜, 시간을 숫자로 반환 (ms초 단위 반환, 1s = 1000ms)
...
function asyncWork() {
  ...
  setTimeout(() => {
    ...
    const end = Date.now();
    console.log(end - start + "ms");
  }, 0); // 지연시간 0: 최소 지연 시간(브라우저 마다 약 4ms(0.004s) 정도의 지연시간이 있음)
