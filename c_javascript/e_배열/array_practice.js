...
console.log(filterTen([3, 15, 23, 11, 19, 10, 55, 60])); // [ 15, 23, 11, 19, 55, 60 ]
console.log(filterTen([3, 2, 5, 1, -10, 5, 6])); // []
...
function findAverage(array) {
  ...
  // return sum / length; 위의 2줄과 같음
}

console.log(findAverage([5, 10, 14])); // 9.666


//! 4. 배열 내의 최대값 찾기
// - findMax(array)
// : 배열의 첫 번째 요소를 max라는 변수에 담기
// : 배열을 순회하면서 max보다 큰 값이 있는 경우 max를 해당 값으로 재할당
function findMax(array) {
  ...
  for (let i = 1; i < length; i++) {
    // 첫 번째 요소는 이미 max 변수에 할당되었기 때문에 1부터 시작
    // > 두 번째 요소부터 순회
    if (array[i] > max) {
      max = array[i]; // 재할당
    }
