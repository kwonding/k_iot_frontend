...
// > 모든 객체들이 메서드와 속성을 상속받기 위한 템플릿(틀)으로 프로토타입 객체를 가짐

const dog = {
  bark() {
    console.log('이것은 dog 객체 입니다.');
  }
}

const myDog = Object.create(dog);
myDog.bark(); // 이것은 dog 객체 입니다.
// >> myDog는 dog를 프로토타입으로 삼은 객체 (dog가 부모 객체 역할)

//! 클래스
// : 새로운 객체를 생성하는 템플릿 역할
// > 프로토타입 구조를 클래스라는 이름을 빌려 표현
...
let human1 = new UpperCamelCaseName('권지애', 30, '학생');
let human2 = new UpperCamelCaseName('권재헌', 16, '중학생');

human1.greet(); // 안녕하세요 권지애님
human2.greet(); // 안녕하세요 권재헌님
...
let person = new Person('지애', '권');
console.log(person.name); // { first: '지애', last: '권' }