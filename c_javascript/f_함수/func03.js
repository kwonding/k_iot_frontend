...
function add(a, b) { // cf)자바 - 타입 명시 X
  console.log(a + b);
}

add(3, 7); // 10

//! 2. JS에서의 매개변수(parameter), 인자(argument) 특징
...
//    지정된 수의 파라미터 이상의 인수는 읽히지 X (무시됨)

function introduce(name, age) {
  console.log(`${name} is ${age} years old`);
}

introduce('권지애', 30); // 권지애 is 30 years old
introduce(30, '권지애'); // 30 is 권지애 years old >> 매개변수의 차례로 인자값이 전달됨
...
function subtract(a, b) {
  let result = a - b
  return result;
  console.log('안녕하세요'); // Unreachable code detected. 도달할 수 없는 코드 감지
