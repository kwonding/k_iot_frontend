...
// JSX) <button onClick={}></button> //? camelCase로 작성됨 + on-이벤트이름
...
function J_Handler() {
  ...
  //# Event Handler //
  // 이벤트 핸들러 함수 (이름작성, 팀 컨벤션)
  // - handle + 요소 + 행위 (handleButtonClick)
  ...
  const buttonHandler = () => {
    console.log('부모로부터 전달하는 이벤트 핸들러 함수');
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={(e) => {
        ...
        <button type='submit'>버튼제출</button>
        <input type="submit" />
      </form>
    </div>
  )
