...
function I_Rendering() {
  ...
  // : 요소 개수 변화 O
  // - 조건에 따른 렌더링
  const businessPeople = peopleDescription.filter((person) => person.job === '멘토');
  // 새로운 배열추출
  ...
  // - map()을 통해 여러 요소를 렌더링할 때, key는 React가 어떤 항목이 변경, 추가 또는 삭제되었는지 파악하는 용도로 사용
  //? key의 특징 - 같은 배열 내에서 유일(데이터의 PK값을 주로 사용 | 배열 index값(권장X))
