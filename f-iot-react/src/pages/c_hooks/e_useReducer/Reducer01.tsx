...
// 3) 상태 변경 흐름 명확 - dispatch가 액션을 받아서 reducer에게 전달 > state에 전달
...
// 리듀서 함수 (일반 함수 정의) - 재사용을 위해 원래 파일 분리
export function reducer (state: CountState, action: CountAction): CountState {
  switch(action.type) {
    case 'increment':
      // 리듀서 함수에는 콜백 형태 사용 필요 X (ex. prev => prev + 1)
      // : 현재 상태 state 값을 인자로 직접 전달 받기 때문
