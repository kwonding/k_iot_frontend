...
function Ref01() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    //* 7) 리액트의 상태 변경 요청
    // : 실시간 변경 반영 x
    // - 이벤트가 다 끝나면, 재렌더링 진행 (+ 변경값은 예약됨)
    setText(e.target.value); 

    //* 8) useRef로 만든 참조 객체의 .current값 수정
    // : 일반 JS 객체 속성 변경과 동일
    // - 렌더링 필요없는 대상 (상태 관리와 무관, ref는 화면 갱신 대상이 아님)
    lengthRef.current = e.target.value.length; // 바로 반영됨

    //* 9) 일반 변수에 데이터 수정
    // : 함수 안의 지역 변수 변경 (렌더링 함수 내부에만 존재, 화면과 연결 X)
