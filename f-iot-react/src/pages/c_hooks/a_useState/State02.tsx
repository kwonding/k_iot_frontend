...
// - 주로 UI에서 발생하는 이벤트(click, input창 등)에 반응하여 상태 변화
...
function State02() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => { // React에서 일어남, Change이벤트임, Input, Select에서도 사용가능
    // input 창에 change 변화가 일어나면 처리(handle)할 로직 처리
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    e.preventDefault(); // 폼 기본 제출 동작 방지 (새로고침 & 서버 전송 방지)
  ...
  const handleLoginChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // - value값 - 사용자로부터 입력받는 값, 명칭 상관X
    const { name, value } = e.target;
    // console.log(name);
    // console.log(value);

    setLogin({
      ...login, // id와 pw 속성을 모두 가지는 login 객체 (이전의 값 가져오기)

      // 변경하고자 하는 name 키를 가진 value 값을 변경 (해당 필드만 값 업데이트 됨)
      [name]: value //name에서 value값을 가지고 오려는 것 - name.value(X)
    });
  }
