...
//  >> useState<User>({name: '권지애', age: 30});

// const [state, setState] = useState(); - X, 반드시 함수형 컴포넌트 안에서 사용!
...
function State01() {
  ...
  //@ initialValue : 초기값, 초기 상태를 설정 (생략 가능 - 생략 시 undefined 값)
  // 명명 규칙 : set + 상태명 (name -> setName, count -> setCount, post -> setPost 등)

  //? useState 호출 규칙
  //@ - 반드시 컴포넌트의 최상단에서 호출! (function 안 최상단)
  ...
  //@ 1) 상태 설정 함수를 그대로 사용
  //    : 이전의 상태를 직접 참조
  //    > 주로 현재(이전, 최신)의 값과 관련이 없는 변화가 이루어진 경우 사용
  const handleUpClick = () => {
    // setCount(5); // 상태 설정 함수의 인자값이 상태 변수로 전달!
    // setCount(count + 1); // 0 + 1 === 1
    // setCount(count + 1); // 0 + 1 === 1 // 56번줄에 영향을 받지않음

    //@ 2) 함수형 컴포넌트 사용
    //    : 현재(이전, 최신)의 상태값을 기반으로 상태를 업데이트 하는 경우 사용
    // cf) set- 상태 변경 함수 내에서 콜백 함수 사용
    //     setCount(() => { return })
    //     >> 해당 콜백 함수의 인자는 '상태의 최신값!'
    //     >> prev-상태명 (prevName, prevCount, prevUser 등 / 이전의)
    setCount(prevCount => prevCount + 1); // 0 + 1 === 1
    setCount(prevCount => prevCount + 1); // 1 + 1 === 2 - 65번줄 영향을 받아 2씩 증가함
  }
