...
// - 특정 가게(트럭)의 예약 내역을 불러오고, 선택한 가게(트럭)와 타임슬롯을 관리
...
export interface Reservation {
  ...
  truckId: number; // 가게(truck) 고유 PK 값 (FK)
...
export const useReservationStore = create<ReservationState>((set) => ({
  ...
  async fetchReservationById(truckId, reservationId) {
    try {
      ...
      if (!dto) return null; // dto에 아무것도없으면 메서드 종료
      ...
      set((state) => {
        ...
        return {
          ...
          reservationList: exists
            ...
            : [mapped, ...state.reservationList], // 새롭게 생성
        };
      });
